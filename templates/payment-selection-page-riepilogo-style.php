...
if (false === $preventivo_data) {
    ...
    if (empty($riepilogo_calcoli) || !isset($riepilogo_calcoli['totali'])) {
        ...
        // Estrai i valori dal calcolatore o dai meta aggiornati
        $prezzo_base = $totals['totale_camere'];
        $totale_assicurazioni = $totale_assicurazioni_meta ?: $totals['totale_assicurazioni'];
        
        // 🔧 FIX: Usa i meta separati che abbiamo salvato con recalculate_preventivo_totals
        $totale_aggiunte = floatval($all_meta['_totale_aggiunte_extra'][0] ?? 0);
        $totale_riduzioni = floatval($all_meta['_totale_sconti_riduzioni'][0] ?? 0);
        $totale_costi_extra = $totale_aggiunte - $totale_riduzioni; // Calcola il netto
        ...
        $totale_costi_extra = 0; // Reset per forzare calcolo dai partecipanti
        ...
        if (true) { // Forza sempre il calcolo
            ...
            if (isset($riepilogo_calcoli['partecipanti'])) {
                foreach ($riepilogo_calcoli['partecipanti'] as $categoria => $dati_categoria) {
                    ...
                    if (isset($dati_categoria['costi_extra'])) {
        ...
        // Calcola aggiunte e riduzioni dai costi extra
    ...
    if (function_exists('btr_price_calculator')) {
        ...
        if (!empty($calculator_totals['valid'])) {
            ...
            // Ricalcola aggiunte e riduzioni dopo il calcolo del calcolatore
            $totale_aggiunte = max($totale_costi_extra, 0);
            $totale_riduzioni = abs(min($totale_costi_extra, 0));
        }
    }
    ...
    // Estrai le variabili per la visualizzazione
    $totale_aggiunte = $preventivo_data['totale_aggiunte'];
    $totale_riduzioni = $preventivo_data['totale_riduzioni'];
    ...
    <div class="btr-section-card">
        ...
        <table class="btr-data-table btr-mt-4">
            ...
            <tbody>
                ...
                <!-- Totale costi extra - Aggiunte -->
                <?php if ($totale_aggiunte > 0): ?>
                <tr>
                    <td><?php esc_html_e('Totale aggiunte extra', 'born-to-ride-booking'); ?></td>
                    <td class="text-right btr-price"><?php echo btr_format_price_i18n($totale_aggiunte); ?></td>
                </tr>
                <?php endif; ?>
                
                <!-- Totale costi extra - Riduzioni -->
                <?php if ($totale_riduzioni > 0): ?>
                <tr>
                    <td><?php esc_html_e('Totale riduzioni/sconti', 'born-to-ride-booking'); ?></td>
                    <td class="text-right btr-price btr-price-discount">-<?php echo btr_format_price_i18n($totale_riduzioni); ?></td>
                </tr>
                <?php endif; ?>
                ...
                <div class="btr-payment-option-card" data-plan="group_split">
                    ...
                    <div id="group-payment-config" class="btr-group-config" style="display: none;">
                        ...
                        <?php if (!empty($adulti_paganti)): ?>
                            ...
                            <div class="btr-group-layout" id="group-payment-area">
                                <div class="btr-group-main" id="group-payment-participants">
                                        ...
                                        >
                                            <div class="btr-participant-info">
                                                ...
                                                <label for="participant_<?php echo $adulto['index']; ?>" class="btr-participant-label">
                                                    <strong class="btr-participant-name"><?php echo esc_html($adulto['nome']); ?></strong>
                                                    <?php if ($adulto['email']): ?>
                            ...
                            <?php endif; ?>
                                ...
                                <aside class="btr-group-side" aria-labelledby="group-summary-title">
                            <div class="btr-group-summary" id="group-summary-card">
                            ...
                            <div id="group-payment-summary" class="btr-summary-card" aria-live="polite" role="status"></div>
...
.btr-participant-selection:hover {
    border-color: rgba(37, 99, 235, 0.35);
    box-shadow: 0 12px 22px -18px rgba(37, 99, 235, 0.45);
}

.btr-participant-selection.is-selected {
    border-color: rgba(37, 99, 235, 0.55);
    background: rgba(37, 99, 235, 0.08);
    box-shadow: 0 22px 35px -22px rgba(37, 99, 235, 0.45);
}

.btr-participant-selection.is-selected .btr-participant-name {
    color: var(--btr-primary);
}

.btr-participant-selection.is-selected .btr-participant-amount {
    color: var(--btr-primary);
    font-weight: 700;
}

.btr-participant-selection:focus-within {
    border-color: rgba(37, 99, 235, 0.55);
    box-shadow: 0 18px 28px -20px rgba(37, 99, 235, 0.45);
}
...
.btr-summary-card {
    ...
    border-radius: var(--btr-radius);
    padding: 1.25rem;
    box-shadow: 0 20px 35px -28px rgba(15, 23, 42, 0.45);
    animation: slideIn 0.3s ease;
}

.btr-summary-card .summary-content {
    display: grid;
    gap: 0.6rem;
    margin-bottom: 0.75rem;
}

.btr-summary-card .summary-row,
.btr-summary-card .summary-highlight {
    display: flex;
    justify-content: space-between;
    align-items: center;
}
...
jQuery(document).ready(function($) {
    ...
    const roundToCent = (value) => Math.round((parseFloat(value) || 0) * 100) / 100;
    ...
    $(document).on('change', '.participant-checkbox', function() {
        const $checkbox = $(this);
        const index = $checkbox.attr('data-index') || $checkbox.closest('tr').attr('data-participant-index');
        ...
        $checkbox.closest('.btr-participant-selection').toggleClass('is-selected', $checkbox.is(':checked'));
    ...
    window.updatePaymentSummary = function updatePaymentSummary() {
        const $selectedAdults = $('.participant-checkbox:checked');
        const selectedCount = $selectedAdults.length;
        const grandTotal = parseFloat($('#btr-payment-plan-selection').data('total') || '0');

        let $summary = $('#group-payment-summary');
        if (!$summary.length) {
            const $summaryAnchor = $('.btr-group-summary').first();
            if ($summaryAnchor.length) {
                $summaryAnchor.after('<div id="group-payment-summary" class="btr-summary-card" aria-live="polite" role="status"></div>');
                $summary = $('#group-payment-summary');
            } else {
                return;
            }
        }
        ...
        if (!selectedCount) {
            $summary.stop(true, true).slideUp(150).attr('aria-hidden', 'true').empty();
            return;
        }
        ...
        $selectedAdults.each(function() {
            const index = $(this).data('index');
            const $row = $('.btr-participant-selection[data-participant-index="' + index + '"]');
            const name = $.trim($row.find('.btr-participant-name').text()) || '<?php echo esc_js(__('Partecipante', 'born-to-ride-booking')); ?> ' + index;
            const amount = roundToCent(parseFloat($row.data('computed-total') || '0'));

            assignedTotal += amount;

            detailsHtml += `
                <li class="summary-detail-row">
                    <span class="summary-detail-name">${name}</span>
                    <span class="summary-detail-amount">${formatPrice(amount)}</span>
                </li>
            `;
        });

        const difference = roundToCent(grandTotal - assignedTotal);
        const perPerson = selectedCount ? roundToCent(assignedTotal / selectedCount) : 0;
        const balanced = Math.abs(difference) <= 0.01;

        const statusText = balanced
            ? '<?php echo esc_js(__('Totale perfettamente distribuito.', 'born-to-ride-booking')); ?>'
            : (difference > 0
                ? '<?php echo esc_js(__('Mancano da assegnare', 'born-to-ride-booking')); ?> ' + formatPrice(difference)
                : '<?php echo esc_js(__('Hai assegnato in eccesso', 'born-to-ride-booking')); ?> ' + formatPrice(Math.abs(difference)));

        const summaryHtml = `
            <div class="summary-content">
                <div class="summary-row"><span><strong><?php echo esc_js(__('Partecipanti selezionati', 'born-to-ride-booking')); ?></strong></span><span>${selectedCount}</span></div>
                <div class="summary-row"><span><?php echo esc_js(__('Totale preventivo', 'born-to-ride-booking')); ?></span><span>${formatPrice(grandTotal)}</span></div>
                <div class="summary-row"><span><?php echo esc_js(__('Totale assegnato', 'born-to-ride-booking')); ?></span><span>${formatPrice(assignedTotal)}</span></div>
                <div class="summary-row summary-diff"><span><?php echo esc_js(__('Saldo residuo', 'born-to-ride-booking')); ?></span><span>${formatPrice(difference)}</span></div>
            </div>
            <div class="summary-highlight">
                <span><?php echo esc_js(__('Quota media selezionati', 'born-to-ride-booking')); ?></span>
                <span class="summary-highlight-value">${formatPrice(perPerson)}</span>
            </div>
            <div class="summary-status ${balanced ? 'ok' : 'warning'}" data-summary-status="true">${statusText}</div>
            <ul class="summary-detail-list">${detailsHtml}</ul>
        `;
    ...
    $('.participant-checkbox:checked').each(function() {
        $(this).closest('.btr-participant-selection').addClass('is-selected');
    });
