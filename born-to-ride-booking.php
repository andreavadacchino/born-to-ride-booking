...
/**
 * Funzione helper per formattazione prezzi con supporto per valori negativi
 * @param float $price Il prezzo da formattare
 * @param bool $show_currency Se mostrare il simbolo €
 * @return string Prezzo formattato (es: €1.000,50 o -€35,00)
 */
if ( ! function_exists( 'btr_format_price' ) ) {
    function btr_format_price( $price, $show_currency = true ) {
        $is_negative = $price < 0;
        $abs_price = abs( $price );
        
        // Formatta il numero con separatori italiani
        $formatted = number_format( $abs_price, 2, ',', '.' );
        
        if ( $show_currency ) {
            // Per valori negativi: -€35,00
            // Per valori positivi: €35,00
...
/**
 * Funzione helper per formattazione prezzi localizzata WordPress con formato italiano
 * Utilizza le impostazioni di localizzazione di WordPress ma forza il formato italiano
 * 
 * @param float $amount Importo da formattare
 * @param bool $show_currency Mostra simbolo € (default: true)
 * @param bool $prefix_currency € come prefisso se true, suffisso se false (default: true)
 * @return string Prezzo formattato localizzato italiano
 */
if ( ! function_exists( 'btr_format_price_i18n' ) ) {
    function btr_format_price_i18n( $amount, $show_currency = true, $prefix_currency = true ) {
        $amount = (float) $amount;
        
        // CORREZIONE: Gestione corretta dei valori negativi
        $is_negative = $amount < 0;
        $abs_amount = abs( $amount );
        
        // Usa number_format per l'importo assoluto
        $formatted = number_format( $abs_amount, 2, ',', '.' );
        
        if ( $show_currency ) {
            if ( $prefix_currency ) {
                // Formato corretto per negativi: -€35,00 (non €-35,00)
                $formatted = $is_negative ? '-€' . $formatted : '€' . $formatted;
            } else {
                // Formato suffisso: -35,00 € (non 35,00 -€)
                $formatted = $is_negative ? '-' . $formatted . ' €' : $formatted . ' €';
            }
        } else {
            // Solo il numero con segno corretto
            $formatted = $is_negative ? '-' . $formatted : $formatted;
        }
