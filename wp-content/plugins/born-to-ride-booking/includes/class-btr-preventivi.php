...
class BTR_Preventivi
{
    ...
    {
        // SECURITY FIX v1.0.216: Rate limiting per endpoint AJAX
        $this->enforce_rate_limit();
        
        // Log inizio funzione (solo in debug mode)
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log('[BTR] Inizio creazione preventivo per: ' . (sanitize_email($_POST['cliente_email'] ?? 'N/A')));
        }

        // SECURITY FIX v1.0.216: Controlla il nonce e CSRF protection
        if (!isset($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], self::NONCE_ACTION_CREATE)) {
            error_log('[BTR SECURITY] Tentativo accesso con nonce non valido da IP: ' . $this->get_client_ip());
            wp_send_json_error(['message' => __('Token di sicurezza non valido.', 'born-to-ride-booking')]);
        }
        
        // SECURITY FIX v1.0.216: Verifica capabilities per operazioni sensibili
        if (!$this->verify_user_capabilities()) {
            error_log('[BTR SECURITY] Tentativo accesso non autorizzato da IP: ' . $this->get_client_ip());
            wp_send_json_error(['message' => __('Accesso non autorizzato.', 'born-to-ride-booking')]);
        }
        ...
        // SECURITY FIX v1.0.216: Gestisci camere selezionate con sanitizzazione sicura
        $camere_selezionate = $_POST['camere'] ?? [];
        if (is_string($camere_selezionate)) {
            // Usa wp_unslash invece di stripslashes per WordPress compatibility
            $json_data = wp_unslash($camere_selezionate);
            
            // SECURITY: Verifica dimensione JSON per evitare payload bomb
            if (strlen($json_data) > 1048576) { // 1MB limit
                error_log('[BTR SECURITY] Payload JSON troppo grande da IP: ' . $this->get_client_ip());
                wp_send_json_error(['message' => __('Dati troppo grandi.', 'born-to-ride-booking')]);
                return;
            }
            
            $camere_selezionate = json_decode($json_data, true);
            
            if (json_last_error() !== JSON_ERROR_NONE) {
                error_log("[BTR SECURITY] Tentativo injection JSON da IP: " . $this->get_client_ip() . " - Errore: " . json_last_error_msg());
                wp_send_json_error(['message' => __('Formato dati non valido.', 'born-to-ride-booking')]);
                return;
            }
            
            // SECURITY: Valida struttura JSON per evitare injection
            $camere_selezionate = $this->sanitize_rooms_data($camere_selezionate ?: []);
        }
        ...
        // SECURITY FIX v1.0.216: Gestione sicura payload JSON completi
        $booking_data_json = [];
        if (isset($_POST['booking_data_json'])) {
            $json_data = wp_unslash($_POST['booking_data_json']);
            
            // SECURITY: Verifica dimensione payload
            if (strlen($json_data) > 2097152) { // 2MB limit
                error_log('[BTR SECURITY] Booking payload troppo grande da IP: ' . $this->get_client_ip());
                wp_send_json_error(['message' => __('Dati prenotazione troppo grandi.', 'born-to-ride-booking')]);
                return;
            }
            
            $booking_data_json = json_decode($json_data, true);
            
            if (json_last_error() !== JSON_ERROR_NONE) {
                error_log("[BTR SECURITY] Tentativo injection booking JSON da IP: " . $this->get_client_ip() . " - Errore: " . json_last_error_msg());
                wp_send_json_error(['message' => __('Formato dati prenotazione non valido.', 'born-to-ride-booking')]);
                return;
            }
            
            // SECURITY: Sanitize complete booking payload
            $booking_data_json = $this->sanitize_booking_payload($booking_data_json);
            
            // Validazione schema base
            $required_fields = ['pacchetto_id', 'totale_preventivo'];
            foreach ($required_fields as $field) {
                if (!isset($booking_data_json[$field])) {
                    error_log("[BTR ERROR] Campo mancante nel JSON: $field");
                    wp_send_json_error(['message' => "Dati incompleti: campo mancante $field"]);
                    return;
                }
            }
        ...
        // SECURITY FIX v1.0.216: Gestione sicura costi_extra_durata
        $costi_extra_durata = [];
        if (isset($_POST['costi_extra_durata'])) {
            $json_data = wp_unslash($_POST['costi_extra_durata']);
            $costi_extra_durata = json_decode($json_data, true);
            
            if (json_last_error() !== JSON_ERROR_NONE) {
                error_log("[BTR ERROR] Errore decodifica costi_extra_durata: " . json_last_error_msg());
                wp_send_json_error(['message' => 'Dati costi extra non validi: ' . json_last_error_msg()]);
                return;
            }
            
            $costi_extra_durata = $costi_extra_durata ?: [];
        }
        ...
        if (!empty($child_labels)) {
            // SECURITY FIX v1.0.216: Sanitize child labels before saving
            $child_labels_sanitized = $this->sanitize_child_labels($child_labels);
            update_post_meta($preventivo_id, '_child_category_labels', $child_labels_sanitized);
            btr_debug_log('[BTR v1.0.186] Saved _child_category_labels: ' . print_r($child_labels_sanitized, true));
        }
        ...
        // UNIFIED CALCULATOR v2.0: Usa sempre il calcolo unificato
        if (class_exists('BTR_Unified_Calculator')) {
            $unified_calculator = new BTR_Unified_Calculator();
            
            // Prepara dati per il calcolo unificato
            $calculation_data = [
                'pricing_totale_camere' => $totale_camere,
                'pricing_num_adults' => $num_adults,
                'pricing_num_children' => $final_num_children,
                ...
                'supplemento_per_persona' => floatval($camere_sanitizzate[0]['supplemento'] ?? 0),
                'notti_base' => 1 // TODO: Recuperare dalle camere
            ];
            
            try {
                $unified_result = $unified_calculator->calculate_unified_total($calculation_data);
                $prezzo_totale = $unified_result['totale_generale'];
                error_log("[BTR UNIFIED CALCULATOR v2.0] Totale calcolato: €{$prezzo_totale}");
                error_log("[BTR UNIFIED CALCULATOR v2.0] Breakdown: " . json_encode($unified_result));
            } catch (Exception $e) {
                error_log("[BTR UNIFIED CALCULATOR] Errore: " . $e->getMessage());
                // Fallback al calcolo precedente
                $prezzo_totale = $totale_camere + $extra_night_total_corrected;
                error_log("[BTR] FALLBACK: Usando totale calcolato: €{$prezzo_totale}");
            }
        } else {
            // Fallback se Unified Calculator non disponibile  
            $prezzo_totale = $totale_camere + $extra_night_total_corrected;
            error_log("[BTR] NO UNIFIED CALCULATOR: Usando totale calcolato: €{$prezzo_totale}");
        }
   ...
   /**
    ...
    /**
     * SECURITY METHODS - Added v1.0.216
     */
    
    /**
     * Enforce rate limiting per AJAX endpoints
     */
    private function enforce_rate_limit() {
        $client_ip = $this->get_client_ip();
        $rate_limit_key = 'btr_rate_limit_' . md5($client_ip);
        
        $current_requests = get_transient($rate_limit_key) ?: 0;
        
        // Limite: 10 richieste per minuto per IP
        if ($current_requests >= 10) {
            error_log('[BTR SECURITY] Rate limit exceeded for IP: ' . $client_ip);
            wp_send_json_error(['message' => __('Troppe richieste. Riprova tra qualche minuto.', 'born-to-ride-booking')]);
        }
        
        set_transient($rate_limit_key, $current_requests + 1, 60); // 60 secondi
    }
    ...
    private function get_client_ip() {
        ...
        foreach ($ip_keys as $key) {
            if (!empty($_SERVER[$key])) {
                ...
                // Per X-Forwarded-For, prendi il primo IP
                if (strpos($ip, ',') !== false) {
     ...
     */
    private function verify_user_capabilities() {
        // Per utenti loggati, verifica capabilities
        if (is_user_logged_in()) {
            return current_user_can('read'); // Capability minima
        }
        
        // Per utenti non loggati, verifica honeypot e timing
        return $this->verify_anonymous_user();
    }
    
    /**
     * Additional verification for anonymous users
     */
    private function verify_anonymous_user() {
        // Verifica honeypot (campo nascosto che dovrebbe rimanere vuoto)
        if (!empty($_POST['btr_honeypot'])) {
            error_log('[BTR SECURITY] Honeypot triggered from IP: ' . $this->get_client_ip());
            return false;
        }
        
        // Verifica timing attack (form submitted troppo velocemente)
        $form_start_time = intval($_POST['btr_form_time'] ?? 0);
        if ($form_start_time > 0 && (time() - $form_start_time) < 3) {
            error_log('[BTR SECURITY] Form submitted too quickly from IP: ' . $this->get_client_ip());
            return false;
        }
    ...
    private function sanitize_rooms_data($rooms) {
        ...
        foreach ($rooms as $index => $room) {
            ...
            $sanitized_room = [
                'adults' => intval($room['adults'] ?? 0),
                'children_f1' => intval($room['children_f1'] ?? 0),
                'children_f2' => intval($room['children_f2'] ?? 0),
                'children_f3' => intval($room['children_f3'] ?? 0),
                'children_f4' => intval($room['children_f4'] ?? 0),
                'assigned_child_f1' => intval($room['assigned_child_f1'] ?? 0),
                'assigned_child_f2' => intval($room['assigned_child_f2'] ?? 0),
                'assigned_child_f3' => intval($room['assigned_child_f3'] ?? 0),
                'assigned_child_f4' => intval($room['assigned_child_f4'] ?? 0),
                'room_type' => sanitize_text_field($room['room_type'] ?? 'doppia'),
                'supplemento' => floatval($room['supplemento'] ?? 0),
                'room_price' => floatval($room['room_price'] ?? 0)
            ];
            
            // Validation: adults must be >= 1
            if ($sanitized_room['adults'] < 1) {
                $sanitized_room['adults'] = 1;
            }
            
            // Validation: max reasonable values
            if ($sanitized_room['adults'] > 10) $sanitized_room['adults'] = 10;
            if ($sanitized_room['supplemento'] > 9999) $sanitized_room['supplemento'] = 9999;
            if ($sanitized_room['room_price'] > 99999) $sanitized_room['room_price'] = 99999;
    ...
    /**
     * Sanitize complete booking payload
     */
    private function sanitize_booking_payload($payload) {
        if (!is_array($payload)) {
            return [];
        }
        ...
        $sanitized = [];
        $allowed_keys = [
            'package_id', 'product_id', 'variant_id', 'num_adults', 'num_children', 'num_infants',
            'total_price', 'base_price', 'extra_night_price', 'extra_costs_total',
            'pricing_adults', 'pricing_children_f1', 'pricing_children_f2', 'pricing_children_f3', 'pricing_children_f4',
            'rooms', 'extra_costs', 'discount_amount', 'payment_method'
        ];
        ...
        foreach ($allowed_keys as $key) {
            if (isset($payload[$key])) {
                switch ($key) {
                    ...
                    case 'num_children':
                    case 'num_infants':
                        $sanitized[$key] = intval($payload[$key]);
                        break;
                    
                    case 'total_price':
                    case 'base_price':
                    case 'extra_night_price':
                    case 'extra_costs_total':
                    case 'pricing_adults':
                    case 'pricing_children_f1':
                    case 'pricing_children_f2':
                    case 'pricing_children_f3':
                    case 'pricing_children_f4':
                    case 'discount_amount':
                        $sanitized[$key] = floatval($payload[$key]);
                        // Validation: reasonable price limits
                        if ($sanitized[$key] > 999999) $sanitized[$key] = 999999;
                        if ($sanitized[$key] < 0) $sanitized[$key] = 0;
                        break;
                    
                    case 'payment_method':
                        $sanitized[$key] = sanitize_text_field($payload[$key]);
                        break;
                    
                    case 'rooms':
                        $sanitized[$key] = $this->sanitize_rooms_data($payload[$key]);
                        break;
                    
                    case 'extra_costs':
                        $sanitized[$key] = $this->sanitize_extra_costs($payload[$key]);
                        break;
    ...
    /**
     * Sanitize extra costs data
     */
    private function sanitize_extra_costs($extra_costs) {
        ...
        foreach ($extra_costs as $cost) {
            ...
            $sanitized[] = [
                ...
                'price' => floatval($cost['price'] ?? 0),
                'quantity' => intval($cost['quantity'] ?? 1),
                'type' => sanitize_text_field($cost['type'] ?? 'per_person')
            ];
        }
    ...
    /**
     * Sanitize child labels
     */
    private function sanitize_child_labels($labels) {
        if (!is_array($labels)) {
            return [];
        }
        
        $sanitized = [];
        $allowed_keys = ['f1', 'f2', 'f3', 'f4'];
