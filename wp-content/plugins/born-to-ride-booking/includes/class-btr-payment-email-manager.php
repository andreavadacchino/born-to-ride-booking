...
class BTR_Payment_Email_Manager {
    ...
    private function __construct() {
        ...
        add_action('btr_payment_completed', [$this, 'send_organizer_notification'], 10, 2); // Notifica organizzatore
    ...
    /**
     * Invia notifica all'organizzatore quando un partecipante completa il pagamento
     * 
     * @param int $payment_id ID del pagamento completato
     * @param int $order_id ID dell'ordine WooCommerce del partecipante
     * @since 1.0.240
     */
    public function send_organizer_notification($payment_id, $order_id) {
        global $wpdb;
        
        btr_debug_log('BTR Email Manager: Invio notifica organizzatore per pagamento ' . $payment_id);
        ...
        if (!$payment) {
            btr_debug_log('BTR Email Manager: Pagamento non trovato');
            return false;
        }
        
        // Recupera l'ordine organizzatore collegato
        $organizer_order_id = $payment->organizer_order_id;
        if (!$organizer_order_id) {
            // Prova a recuperare dall'ordine del preventivo
            $organizer_order_id = get_post_meta($payment->preventivo_id, '_btr_organizer_order_id', true);
        }
        
        if (!$organizer_order_id) {
            btr_debug_log('BTR Email Manager: Ordine organizzatore non trovato per preventivo ' . $payment->preventivo_id);
            return false;
        }
        
        // Recupera l'ordine organizzatore
        $organizer_order = wc_get_order($organizer_order_id);
        if (!$organizer_order) {
            btr_debug_log('BTR Email Manager: Ordine organizzatore non valido: ' . $organizer_order_id);
            return false;
        }
        
        // Recupera email organizzatore
        $organizer_email = $organizer_order->get_billing_email();
        if (!$organizer_email) {
            btr_debug_log('BTR Email Manager: Email organizzatore non trovata');
            return false;
        }
        ...
        // Calcola stato pagamenti gruppo
        $payment_stats = $this->get_group_payment_stats($preventivo_id);
        
        // Prepara soggetto email
        $subject = sprintf(
            __('Pagamento Ricevuto - %s ha pagato la sua quota', 'born-to-ride-booking'),
        ...
        $template_data = [
            ...
            'amount_paid' => btr_format_price_i18n($payment->amount),
            'package_title' => $package_title,
            'payment_stats' => $payment_stats,
            'dashboard_url' => $this->get_organizer_dashboard_url($organizer_order_id),
            'preventivo_id' => $preventivo_id,
            'order_id' => $order_id,
            'payment_date' => current_time('d/m/Y H:i')
        ];
        
        // Genera contenuto email
        $content = $this->get_email_template('organizer-payment-notification', $template_data);
        
        // Invia email
        $sent = $this->send_email($organizer_email, $subject, $content);
        
        if ($sent) {
            btr_debug_log('BTR Email Manager: Notifica inviata a organizzatore: ' . $organizer_email);
            
            // Aggiungi nota all'ordine organizzatore
            $organizer_order->add_order_note(sprintf(
                __('Notifica inviata: %s ha completato il pagamento di €%s', 'born-to-ride-booking'),
                $payment->participant_name,
                number_format($payment->amount, 2, ',', '.')
            ));
        } else {
            btr_debug_log('BTR Email Manager Error: Impossibile inviare notifica a: ' . $organizer_email);
        }
    ...
    /**
     * Calcola statistiche pagamenti gruppo
     * 
     * @param int $preventivo_id
     * @return array
     */
    private function get_group_payment_stats($preventivo_id) {
        global $wpdb;
        ...
        return [
            ...
            'pending_count' => intval($stats->total_participants - $stats->paid_count),
    ...
    /**
     * Ottiene URL dashboard organizzatore
     * 
     * @param int $order_id
     * @return string
     */
    private function get_organizer_dashboard_url($order_id) {
        // Per ora restituisce la pagina account WooCommerce
        // TODO: Implementare dashboard dedicata
        $account_page_url = wc_get_page_permalink('myaccount');
        $order_url = wc_get_endpoint_url('view-order', $order_id, $account_page_url);
        
        return apply_filters('btr_organizer_dashboard_url', $order_url, $order_id);
    }
