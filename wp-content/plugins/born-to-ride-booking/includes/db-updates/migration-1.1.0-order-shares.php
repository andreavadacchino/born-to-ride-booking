...
class BTR_Migration_1_1_0 {
    ...
    public function up() {
        ...
        // Verifica che la tabella sia stata creata - use prepared statement
    ...
    public function down() {
        ...
        $backup_table = $wpdb->prefix . 'btr_order_shares_rollback_' . date('YmdHis');
    ...
    private function add_foreign_keys() {
        ...
        if (!$fk_exists) {
            try {
                ...
                $sanitized_posts_table = preg_replace('/[^a-zA-Z0-9_]/', '', $posts_table);
                $wpdb->query(
                    "ALTER TABLE `{$sanitized_table_name}` 
                    ADD CONSTRAINT fk_order_shares_order 
                    FOREIGN KEY (order_id) REFERENCES `{$sanitized_posts_table}`(ID) 
