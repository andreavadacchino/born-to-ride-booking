jQuery(document).ready(function ($) {
    ...
    window.btrBookingState = {
        ...
        recalculateTotal: function() {
            ...
            // UNIFIED CALCULATOR v1.0.216: Validazione automatica ogni 2 secondi
        ...
        // UNIFIED CALCULATOR v1.0.216: Sincronizzazione calcoli frontend-backend
        validateWithUnifiedCalculator: function() {
            ...
            return new Promise((resolve, reject) => {
                $.ajax({
                    ...
                    success: (response) => {
                        console.log('[UNIFIED CALCULATOR] Frontend validation:', response);
                        ...
                        if (discrepanza > 0.01) {
                            console.warn('[SPLIT-BRAIN] Discrepanza rilevata:', {
                                frontend: frontendTotal,
                                backend: backendTotal,
                                discrepanza: discrepanza
                            });
                            
                            // Aggiorna stato frontend con valori backend
    ...
    `;
                    ...
                    for (const roomType in childF3ByRoomType) {
                        ...
                        if (data.count > 0) {
                            ...
                            if (extraNightFlag && extraNightPP > 0) {
                                const childExtraF3 = response.data.rooms?.[0]?.extra_night_child_f3 || (extraNightPP * 0.5); // Usa prezzo dal backend o fallback 50%
                                const extraNightsCount = (typeof window.btrExtraNightsCount === 'number' && window.btrExtraNightsCount > 0) ? window.btrExtraNightsCount : 1; // Default a 1 solo per visualizzazione
                    ...
                    for (const roomType in childF4ByRoomType) {
                        ...
                        if (data.count > 0) {
                            ...
                            if (extraNightFlag && extraNightPP > 0) {
                                const childExtraF4 = response.data.rooms?.[0]?.extra_night_child_f4 || (extraNightPP * 0.5); // Usa prezzo dal backend o fallback 50%
                                const extraNightsCount = (typeof window.btrExtraNightsCount === 'number' && window.btrExtraNightsCount > 0) ? window.btrExtraNightsCount : 1; // Default a 1 solo per visualizzazione
